#!/bin/bash
#
# Init Consul k/v store based on json config file
# Requires: jq - a lightweight and flexible command-line JSON processor.
#
# Author: https://raw.githubusercontent.com/shufo/nginx-consul-template/master/consul_config.sh

CONFIG_JSON=${CONFIG_JSON:-/config.json}
CONFIG=$(cat ${CONFIG_JSON})
CONSUL_KV_PREFIX=${CONSUL_KV_PREFIX:-nginx}
CONSUL_PORT_8500_TCP_ADDR=${CONSUL_PORT_8500_TCP_ADDR:-consul}
